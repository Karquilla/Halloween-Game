...
let gameState = 'start'; // start, title, run, stage clear/ game over.
let levelNum = 0;
...
function setup()
{
    ...
    frameRate(60);
    world.gravity.y = 2;
    level1 = new level(lvl1ShotCount);
    shot = new Shot;
    //temp run for debug
    gameState = 'run';
...
function Level1()
{
    ...
    {
        level1.create_platform(1000,50,100)
        level1.create_platform(1500,75,200)
        level1.isSetup = true;
    }
...
class Shot
{
    constructor()
    {
        ...
        this.sightline = new Sprite(this.originX,this.originY,100,5,"n");
        this.sightline.img = sightlineImg;
        this.sightline.offset.x = -100;
        this.sprite.overlaps(this.sightline);
        this.barsprite = new Sprite(this.originX - 200,this.originY,100,25,'n');
        this.barmaxsprite = new Sprite(this.originX - 100,this.originY,5,25,'n')
        this.r = 0
        this.g = 200
...
class level
{
    ...
    target_is_destroyed()
    {
        for (var i = 0; i < this.targets.length; i++)
        {
            if (this.targets[i].collides(this.floor))
            {
